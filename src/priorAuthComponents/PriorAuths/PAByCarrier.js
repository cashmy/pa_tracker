...
export default function PAByCarrier() {

    
    const location = useLocation()
    const { workingCarrierId } = location.state
    
    const [carrierName, setCarrierName] = useState("")
    const [carrierShortName, setCarrierShortName] = useState("")
    ...
    async function getPAs() {
        try {
            const response = await PriorAuthService.getAllPAsbyCarrierAndArchive(workingCarrierId, archiveStatus);
    ...
    async function getCarrier() {
        try {
            const response = await CarrierService.getCarrier(workingCarrierId);
            setCarrierName(response.data.carrierName)
            setCarrierShortName(response.data.carrierShortName)
            console.log("Carrier Info: ",response.data)
        }
        catch (e) {
            console.log('API call unsuccessful', e)
        }
            ...
            fn: items => {
                ...
                if (target.value === "")
                    ...
                    return items.filter(x => (
                        x.patient.patientFirstName.toLowerCase().includes(target.value.toLowerCase()) 
                        || x.patient.patientLastName.toLowerCase().includes(target.value.toLowerCase())
                        || x.provider.providerLastName.toLowerCase().includes(target.value.toLowerCase())
                        || x.status.statusName.toLowerCase().includes(target.value.toLowerCase())
                        || x.paTreatmentCode.toLowerCase().includes(target.value.toLowerCase())
                    ))
            }
        })
    }
    ...
    return (
        <>
            <PageHeader
                title={`Prior Authorizations for Carrier:   ${carrierShortName} - ${carrierName}`}
                                    ...
                                    <TableCell>
                                        ...
                                        <Controls.ActionButton
                                            ...
                                            onClick={() => {
                                                setConfirmDialog({
                                                    isOpen:true,
                                                    title:"Are you sure you want to delete this Prior Authorization?",
